...
def encoder(rgb, num_of_resblock=9, h_dim=128, is_train=True, reuse=False):
...
def generator_dcgan(feat_vec, hidden_dim=128, is_train=True, reuse=False):
    ...
    with tf.variable_scope('generator', reuse=reuse):
        ...
        # (8, 8, 512)
        net_h1 = DeConv2d(net_h0, n_filter=gf_dim*8, filter_size=filter_size, strides=strides,
                          padding='SAME', W_init=w_init, name='g/h1/deconv2d')

        net_h1 = BatchNormLayer(net_h1, act=tf.nn.relu, is_train=is_train,
                                gamma_init=g_init, name='g/h1/batch_norm')

        # (16, 16, 256)
        net_h2 = DeConv2d(net_h1, n_filter=gf_dim*4, filter_size=filter_size, strides=strides,
                          padding='SAME', W_init=w_init, name='g/h2/deconv2d')

        net_h2 = BatchNormLayer(net_h2, act=tf.nn.relu, is_train=is_train,
                                gamma_init=g_init, name='g/h2/batch_norm')

        # (32, 32, 128)
        net_h3 = DeConv2d(net_h2, n_filter=gf_dim*2, filter_size=filter_size, strides=strides,
                          padding='SAME', W_init=w_init, name='g/h3/deconv2d')

        net_h3 = BatchNormLayer(net_h3, act=tf.nn.relu, is_train=is_train,
                                gamma_init=g_init, name='g/h3/batch_norm')

        # (64, 64, 64)
        net_h4 = DeConv2d(net_h3, n_filter=gf_dim, filter_size=filter_size, strides=strides,
                          padding='SAME', W_init=w_init, name='g/h4/deconv2d')

        net_h4 = BatchNormLayer(net_h4, act=tf.nn.relu, is_train=is_train,
                                gamma_init=g_init, name='g/h4/batch_norm')

        # (128, 128, 32)
        net_h5 = DeConv2d(net_h4, n_filter=gf_dim//2, filter_size=filter_size, strides=strides,
                          padding='SAME', W_init=w_init, name='g/h5/deconv2d')

        net_h5 = BatchNormLayer(net_h5, act=tf.nn.relu, is_train=is_train,
                                gamma_init=g_init, name='g/h5/batch_norm')

        # (256, 256, 3)
        net_h6 = DeConv2d(net_h5, n_filter=c_dim, filter_size=filter_size, strides=strides,
                          padding='SAME', W_init=w_init, name='g/h6/deconv2d')

        net_h6.outputs = tf.nn.tanh(net_h6.outputs)

        return net_h6, net_h6.outputs
...
def discriminator(inputs, is_train=True, reuse=False):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        # (8, 8, 1024)
        net_h4 = Conv2d(net_h3, n_filter=df_dim*16, filter_size=filter_size, strides=strides,
                        act=None, padding='SAME', W_init=w_init, name='d/h4/conv2')
        ...
        net_h5 = DenseLayer(net_h5, n_units=1, act=tf.identity, W_init=w_init, name='d/h4/lin_sigmoid')
